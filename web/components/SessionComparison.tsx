...
function identityStatus(session: SessionDetailResponse): string {
  return "matchStatus" in session.assessment.identity
    ? session.assessment.identity.matchStatus
    : "unavailable";
}
...
function evidenceSimilarity(
  ...
  const evidence = identity.evidence.find(
    (item) => item.evidenceClass === evidenceClass,
  );
...
export function SessionComparison({ sessions }: SessionComparisonProps) {
  ...
  const rows = [
    ...
    [
      "Score confidence",
      `${Math.round(left.humanConfidence * 100)}%`,
      `${Math.round(right.humanConfidence * 100)}%`,
    ],
    ...
    ...(["behavior", "device", "context", "network"] as const).map(
      (evidenceClass) => [
  ...
  return (
    <section
      className={styles["comparison-panel"]}
      data-evidence-panel
    ...
    >
      ...
      <div className={styles["comparison-scroll"]}>
        <table>
          <thead>
            <tr>
              <th scope="col">Measure</th>
              <th scope="col">
                <code>{left.sessionId}</code>
              </th>
              <th scope="col">
                <code>{right.sessionId}</code>
              </th>
            </tr>
          </thead>
          <tbody>
